...
inline LL update(LL ql, LL qr, LL nowl, LL nowr, LL num, LL delta1, LL delta2) {
    ...
    if (ql > nowr || nowl > qr) return 0;
    if (ql <= nowl && nowr <= qr) {
        tree[num] = ( tree[num] + get(delta1, delta2, nowr-nowl + 3) + p - delta2 )% p;
...
inline void pushdown(LL num, LL l) {
	...
	tree[num << 1] = (tree[num << 1] + get(mrka[num], mrkb[num], l / 2 + 2) + p - mrkb[num]) % p;
	tree[num << 1 | 1] = (tree[num << 1 | 1] + get(mrka[num], mrkb[num], l + 2) + p - get(mrka[num], mrkb[num], l / 2 + 2)) % p;
	mrka[num] = mrkb[num] = 0;
	return;
}
