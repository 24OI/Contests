...
{
	fscore=fopen(argv[5],"w");//打开得分文件
	freport=fopen(argv[6],"w");//打开报告文件
	fstd=fopen(argv[3],"r");//打开测试点标准输出文件
	int score=atoi(argv[4]);//取得测试点的分数
	
	fin=fopen(argv[1],"r");//打开测试点标准输入文件
	fout=fopen(argv[2],"r");//打开用户的数据输出文件
	if (!fout)
	{
